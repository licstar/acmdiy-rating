...
public sealed class PreProcessUtility
{
   ...
   public static string HTML2Text(string content)
   {
      ...
      //将换行符更换成回车换行标记
      content = Regex.Replace(content, @"<br>", Environment.NewLine, RegexOptions.IgnoreCase);
      content = Regex.Replace(content, @"</br>", Environment.NewLine, RegexOptions.IgnoreCase);
      content = Regex.Replace(content, @"<p>", Environment.NewLine, RegexOptions.IgnoreCase);
      content = Regex.Replace(content, @"</p>", Environment.NewLine, RegexOptions.IgnoreCase);
      content = Regex.Replace(content, @"<p( )*/>", Environment.NewLine, RegexOptions.IgnoreCase);
      content = Regex.Replace(content, @"<br( )*/>", Environment.NewLine, RegexOptions.IgnoreCase);
      ...
      //去掉空行
      content = Regex.Replace(content, @"\r[\s| ]*\n", Environment.NewLine);
